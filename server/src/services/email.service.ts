...
export async function sendWelcomeEmail(to: string, name: string): Promise<void> {
  const html = wrap(
    ...
    `<p style="color:#475569">Вы успешно зарегистрировались на платформе <strong>AI Job Search</strong>.</p>
...
export async function sendNewApplicationEmail(
  ...
  const html = wrap(
    ...
    `<p style="color:#475569">Соискатель <strong>${seekerName}</strong> откликнулся на вакансию <strong>«${vacancyTitle}»</strong>.</p>
...
export async function sendApplicationStatusEmail(
  ...
  const statusMap: Record<string, { subject: string; text: string; color: string }> = {
    VIEWED: {
  ...
  const html = wrap(
    info.subject,
    `<p style="color:#475569">${info.text}</p>`
  );
...
export async function sendInterviewInvitationEmail(
  ...
  const html = wrap(
    ...
    `<p style="color:#475569">Уважаемый(-ая) <strong>${seekerName}</strong>,</p>
...
export async function sendPasswordResetEmail(to: string, token: string): Promise<void> {
  ...
  const html = wrap(
    'Сброс пароля',
    `<p style="color:#475569">Мы получили запрос на сброс пароля для вашей учётной записи.</p>
